...
import com.andikas.storyapp.data.source.local.entity.RemoteKeysEntity
...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val responseData =
                apiService.getPagingStories(page = page, size = state.config.pageSize).listStory?.map {
                    StoryEntity(it.id, it.name, it.description, it.photoUrl)
                }
            val endOfPaginationReached = responseData?.isEmpty() as Boolean
