...
class EmailValidator(
    ...
    override fun validate(validatorResult: ValidatorResult) {
        editText.doAfterTextChanged {
            ...
            if (text.isNotEmpty()) {
                if (text.validateEmail()) setFieldError(false)
                else setFieldError(true, context.string(R.string.error_email_wrong_format))
            } else {
    ...
    override fun validationResult(): Boolean =
        (editText.text.toString().isNotEmpty() && editText.text.toString().validateEmail())
